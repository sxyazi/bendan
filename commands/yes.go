...
func YesRight(msg *tgbotapi.Message) bool {
	...
	case '是':
		opt = [2][]string{{"是", "对", "是的", "是啊", "是诶"}, {"no", "不是", "不是啊", "不是8", "不是吧", "应该不是"}}
	case '有':
		opt = [2][]string{{"有", "有的", "有啊", "有诶", "有吧"}, {"no", "没有", "没啊", "没8", "没吧", "没有吧", "应该没吧"}}
	case '行':
		opt = [2][]string{{"行", "行啊", "肯定行", "我觉得行"}, {"不行", "不太行", "应该不行", "我觉得不行"}}
	default: // e.g. "应该是", "应该有", etc.
		switch true {
		case strings.Contains(token.Word, "对"): // "X应该对吧"
			opt = [2][]string{{"yy", "yyy", "对的", "挺对的", "没毛病"}, {"不对", "不对啊", "不大对", "明显错了", "肯定不对啊"}}
		...
		default: // "X应该行吧"
			opt = [2][]string{{"yy", "yyy", "可以", "肯定行", "我觉得行"}, {"不行", "不太行", "应该不行", "肯定不行", "我觉得不行"}}
		}
	}
